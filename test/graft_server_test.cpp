...
public:
    ...
    public:
        std::atomic<graft::Server*> pserver{nullptr};
...
private:
    ...
    protected:
        virtual bool onHttpRequest(const http_message *hm, int& status_code, std::string& headers, std::string& data) override
        {
            ...
            graft::Context ctx(mainServer.pserver.load()->getGcm());
...
TEST_F(GraftServerCommonTest, timerEvents)
{
    ...
    auto make_timer = [=,&cntrs_all,&cntrs](int i, int ms)
    {
        ...
        mainServer.pserver.load()->addPeriodicTask(
    ...
    for(int i=0; i<N; ++i)
    {
        ...
        n -= (mainServer.pserver.load()->getCopts().upstream_request_timeout*1000*n)/((i+1)*ms_step);
