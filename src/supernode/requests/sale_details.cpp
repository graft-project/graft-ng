                           ...
                           graft::Context& ctx, graft::Output& output)
{
    ...
    if (have_data_locally) {
        ...
        broadcastReq.params.sender_address = supernode->walletAddress();
        size_t maxIndex = authSample.size() - 1;
        size_t randomIndex = utils::random_number<size_t>(0, maxIndex);
        broadcastReq.params.receiver_addresses.emplace_back( authSample.at(randomIndex)->walletAddress() );
                           ...
                           graft::Context& ctx, graft::Output& output)
{
    ...
    if (broadcastReq.receiver_addresses.size() != 1
            || *broadcastReq.receiver_addresses.begin() != supernode->walletAddress()) {
        std::ostringstream oss;
        oss << "wrong receiver single address, addresses: ";
        for(auto& addr : broadcastReq.receiver_addresses)
        {
            oss << addr << " ";
        }
        oss << ", expected address: " + supernode->walletAddress();
        LOG_ERROR(oss.str());
